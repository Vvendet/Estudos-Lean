...
lemma SCOM_sim_red {α : Type} (R : ARS_Mod α) (h : StronglyCompatibleWithH R) :
  ...
  | tail x1 y1 h_prefix h_step ih =>
    intro z hyz
    -- Aplicamos SCOM H no passo: x1 H y1 → z
    rcases h x1 y1 z h_step hyz with ⟨v, hxv, hvz⟩
...
lemma StronglyCompatible_to_Compatible {α : Type} (R : ARS_Mod α)
  ...
  intro a b c hab hbc
  have hbc_rtg := ReducesStar_iff_ReducesStar'.mp hbc
  clear hbc
  -- Indução sobre a cadeia b →* c
  induction hbc_rtg with
  | refl =>
    exists a
    constructor
    · exact ReducesStar_iff_ReducesStar'.mpr Relation.ReflTransGen.refl
    · exact Relation.ReflTransGen.single hab
  | tail x y _ h_step ih =>
    rcases ih with ⟨d1, had1, hd1x⟩
    -- Invocamos o nosso lema auxiliar cirúrgico
